...
class ChatSambaNovaCloud(BaseChatModel):
    ...
    sambanova_api_key: str = Field(default='')  # Changed from SecretStr to str
    ...
    def __init__(self, **kwargs: Any) -> None:
        """Initialize with environment variables and override with any passed kwargs"""
        # Get API key from environment or kwargs
        api_key = kwargs.get('sambanova_api_key') or os.getenv('SAMBANOVA_API_KEY', '')
        url = kwargs.get('sambanova_url') or os.getenv('SAMBANOVA_URL', 'https://api.sambanova.ai/v1/chat/completions')
    ...
    def _handle_request(self, messages_dicts: List[Dict[str, Any]], stop: Optional[List[str]] = None, streaming: bool = False, **kwargs: Any) -> Response:
        ...
        response = http_session.post(
            self.sambanova_url,
            headers={
                'Authorization': f'Bearer {self.sambanova_api_key}',  # No need to use get_secret_value()
